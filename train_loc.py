...
def train():
    ...
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, collate_fn=loc_collate, num_workers=4)
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, collate_fn=loc_collate, num_workers=4)

    model = LocModel1D(num_anchors_per_sector=loader.NUM_ANCHORS_PER_SECTOR, glob=True).to(DEVICE)
    
    # Load weights từ Backbone của mô hình Segmentation
    ...
    if os.path.exists(PRETRAINED_SEG):
        ...
        # Lọc lấy các layer thuộc backbone
    ...
    with mlflow.start_run(run_name="PPN_Loc_FineTune_v1"):
        mlflow.log_params({
            "model": "LocModel1D",
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            print(f"Epoch {epoch}: Train Loss={avg_train:.4f}, Val Loss={avg_val:.4f}, LR={current_lr:.6f}")
