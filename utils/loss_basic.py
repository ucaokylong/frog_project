...
def dice_loss_torch(y_true, y_pred, smooth=1.0):
    """
    Dice loss để xử lý mất cân bằng nhãn trong segmentation.
    y_true, y_pred: (B, 1, L)
    """
...
def weighted_ce_loss_torch(y_true, logits, beta=1.5):
    """
    Weighted Cross Entropy để ưu tiên nhận diện chân người (nhãn 1).
    """
...
def seg_mixed_loss_torch(y_true, logits):
    """
    Kết hợp Dice và BCE cho bài toán Segmentation.
    """
...
def loc_model_loss_torch(y_true, y_pred):
    """
    Hàm loss cho PPN (Localization) - Đã tối ưu cho cấu trúc Hard Threshold.
    y_true: (B, S, A, 3) -> [objectness, diff_dist, diff_arc]
    """
    gt_obj = y_true[..., 0]
    gt_reg = y_true[..., 1:3]
    pr_obj = y_pred[..., 0]
    ...
    # Mask cho các anchor CÓ NGƯỜI (1.0)
    pos_mask = (gt_obj > 0.5).float()
    num_pos = pos_mask.sum() + 1e-6

    # 1. Classification Loss (BCE thuần)
    # Vì dataloader đã bỏ -1.0, tính trực tiếp mean trên toàn bộ grid anchor
    cls_loss = F.binary_cross_entropy_with_logits(pr_obj, gt_obj, reduction='mean')

    # 2. Regression Loss (Smooth L1 chuẩn)
    sigma_sq = 9.0 # (sigma=3.0)
    ...
    # SỬA LỖI TÁC GIẢ: Nhỏ dùng Quad, Lớn dùng Lin
    loss_all = piecewise * reg_loss_quad + (1.0 - piecewise) * reg_loss_lin
    
    # Chỉ phạt sai số tọa độ trên các anchor trúng người
