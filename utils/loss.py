...
def loc_model_loss_torch(y_true, y_pred):
    ...
    # Mask cho các anchor có người (1.0) và không có người (0.0), bỏ qua (-1.0)
    pos_mask = (gt_obj > 0.5).float()
    valid_mask = (gt_obj >= 0.0).float()
    num_pos = pos_mask.sum() + 1e-6

    # 1. Classification Loss (BCE)
    cls_loss_all = F.binary_cross_entropy_with_logits(pr_obj, (gt_obj > 0.5).float(), reduction='none')
